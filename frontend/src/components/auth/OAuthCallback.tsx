  ...
  useEffect(() => {
    ...
    const handleOAuthCallback = async () => {
      try {
        ...
        const hasOAuthParams = hash.includes('access_token') || search.includes('code') || search.includes('access_token');
        ...
        if (!session) {
          console.error("No session found from OAuth callback");
        ...
        if (!finalAccountType) {
          const state = location.state as { accountType?: string; returnTo?: string } | null;
        ...
        if (finalAccountType) {
          ...
          if (!validAccountTypes.includes(finalAccountType.toLowerCase())) {
            console.error("Invalid account type:", finalAccountType);
        ...
        const response = await fetch("/api/oauth-callback", {
          ...
          body: JSON.stringify({
            ...
            user_metadata: {
              ...
              ...(finalAccountType && { account_type: finalAccountType.toLowerCase() })
            },
          }),
        });
